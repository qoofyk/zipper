...
void make_prefetch_id(GV gv, int cid, int num_int,int* temp_int_pointer){
  ...
  int* temp2 = (int*)(gv->org_recv_buffer+sizeof(char)*gv->block_size);
  for(i=1;i<=num_int;i++){
    temp1[gv->recv_tail]=cid;
    temp1[gv->recv_tail+1]=temp2[i];

    // #ifdef DEBUG_PRINT
    // printf("++++++++++++++++Receiver MIX: written id= %d\n", temp1[gv->recv_tail+1]);
    // fflush(stdout);
    // #endif //DEBUG_PRINT

    // if(temp2[i]>gv->ana_total_blks){
    //   printf("Error! Analysis Process %d Receiver get temp2[i]=%d\n", gv->rank[0], temp2[i]);
    //   fflush(stdout);
    // }
    gv->recv_tail+=2;
  }
...
void analysis_receiver_thread(GV gv,LV lv){
  ...
  while(1){
    ...
    if(status.MPI_TAG==MPI_MSG_TAG){
      ...
      block_id =*((int *)(gv->org_recv_buffer+gv->block_size-8));
      // #ifdef DEBUG_PRINT
      // printf("Analysis Receiver %d Get a Long msg!long_messageind=%d,gv->mpi_recv_progress_counter=%d,block_id=%d\n",
      //   gv->rank[0], long_msg_id,gv->mpi_recv_progress_counter,block_id);
      // fflush(stdout);
      // #endif //DEBUG_PRINT
      ...
      temp_int_pointer = (int*)(gv->org_recv_buffer+sizeof(char)*gv->block_size);
