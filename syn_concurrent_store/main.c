...
int main(int argc, char **argv){
	...
	if(argc != 12) {
		fprintf(stderr, "Usage: %s writer#, reader#, block_size*64KB, num_blks, computer_group_size, analysis_process_num, msleep\n", argv[0]);
		exit(1);
	}
	...
	gv->block_size = atoi(argv[5])*1024*64; //64K B
  	...
  	gv->analysis_data_len = sizeof(char)*gv->block_size+sizeof(int)*(1+1+1); //src,blk_id,writer_state,consumer_state
	...
	if (gv->color == 0){
	    ...
	    if(gv->rank[0]==0){
		    ...
		    printf("Compute Process %d: PRODUCER_Ringbuffer %ldGB, size is %d member\n",
		...
		/* Create threads */
		for(i = 0; i < (gv->compute_writer_num+1+1); i++) {
		  init_lv(lvs+i, i, gv);
	    ...
	    if(gv->rank[0]==gv->compute_process_num || gv->rank[0]==(gv->compute_process_num+gv->analysis_process_num)){
		    ...
		    printf("Analysis Process %d: CONSUMER_Ringbuffer %ldGB, size is %d member\n",
		...
		t3=get_cur_time();
		printf("Analysis Process %d on %s analysis job total time = %f\n",gv->rank[0], gv->processor_name,t3-t2);
	}
	else{
		printf("Error!\n");
	}
