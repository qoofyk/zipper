...
void read_blk(GV gv, LV lv,int last_gen_rank, int blk_id, char* buffer, FILE *fp){
  double t0=0, t1=0;
  int error=-1;
  int i=0;
    ...
    error=fseek(fp, blk_id*gv->block_size, SEEK_SET);
      if(error==-1){
        if(i==TRYNUM-1){
          ...
          printf("Warning: Analysis Process %d fseek error block_id=%d,*fp=%x\n",
            gv->rank[0], blk_id,*fp);
          fflush(stdout);
        }
        i++;
...
void reader_thread(GV gv,LV lv) {
  ...
  while(1){
    ...
    if(flag == 1){
      ...
      *(int *)(new_buffer+sizeof(int))=last_file_position;
      ...
      for(j=0;j<gv->computer_group_size;j++){
        if(open[j]==0 && (last_gen_rank)==((gv->rank[0]-gv->num_compute_nodes)*gv->computer_group_size+j)){
          ...
          while((fp[last_gen_rank%gv->computer_group_size]==NULL) && (i<TRYNUM)){
            fp[last_gen_rank%gv->computer_group_size]=fopen(file_name,"r+");
  ...
  t3 = get_cur_time();
  printf("Analysis Process %d Reader %d total_time= %f, io_read_time = %f, only_fread_time/Block= %f with %d blocks\n",
