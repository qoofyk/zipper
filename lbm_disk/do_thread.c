...
void node1_send(GV gv,LV lv){
  int i,send_counter;
  int disk_msg_id=0;
  int dest=gv->rank[0]/gv->computer_group_size+gv->num_analysis_nodes*gv->computer_group_size;
  double t0 = 0, t1 = 0, t2 = 0, t3 = 0, t4=0,t5=0,wait_lock=0;
  double send_time=0;
  // printf("Compute Process %d Send thread %d start!\n", gv->rank[0], lv->tid);
  //int errorcode;
  ...
  while(1){
    ...
    if(send_counter > 0){
      ...
      //   }
      // }
      // printf("sender %d send to dest %d\n", gv->rank[0],dest);
      MPI_Send(gv->mpi_send_block_id_array, send_counter+1,MPI_INT,dest,MPI_DISK_TAG,MPI_COMM_WORLD);
      disk_msg_id++;
      #ifdef DEBUG_PRINT
      printf("Compute %d Sender disk_msg %d!\n", gv->rank[0], disk_msg_id);
      fflush(stdout);
      #endif //DEBUG_PRINT
      t3 = get_cur_time();
      ...
      if(gv->mpi_send_progress_counter>=gv->cpt_total_blks) break;
    }
  }
  t1 = get_cur_time();
  printf("Compute Process %d Sender thread %d total time is %f, send_time = %f, wait_lock = %f\n", gv->rank[0], lv->tid, t1-t0, send_time, wait_lock);
}
...
void node2_receive(GV gv,LV lv){
  ...
  // printf("Analysis Process %d Receiver thread %d Start receive!\n",gv->rank[0], lv->tid);
  ...
  while(1){
    ...
    //     }
    // }
    MPI_Recv(gv->mpi_recv_block_id_array, gv->cpt_total_blks+1,MPI_INT,MPI_ANY_SOURCE,MPI_DISK_TAG,MPI_COMM_WORLD, &status);
    disk_msg_id++;
    #ifdef DEBUG_PRINT
    printf("Ana %d Receiver disk_msg %d!\n", gv->rank[0],disk_msg_id);
    fflush(stdout);
    #endif //DEBUG_PRINT
    t3 = get_cur_time();
    receive_time += t3-t2;
    ...
    for(j=0;j<recv_counter;j++){
      gv->prefetch_id_array[gv->recv_tail]=status.MPI_SOURCE;
      gv->prefetch_id_array[gv->recv_tail+1]=gv->mpi_recv_block_id_array[j+1];
    ...
    if(gv->recv_progress_counter>=gv->ana_total_blks) {
      //printf("Node 2 receive break!\n");
      break;
    }
  }
  t1 = get_cur_time();
  printf("Analysis Process %d Receiver %d total time = %f, receive_time = %f, wait_lock = %f, disk_msg_id = %d\n",
    gv->rank[0], lv->tid, t1-t0, receive_time, wait_lock, disk_msg_id);
}
