...
void receiver_thread(GV gv,LV lv){
  ...
  while(1){
    ...
    if(status.MPI_TAG==MPI_MSG_TAG){
      ...
      // temp_double = (double*)gv->org_recv_buffer;
      // t3 = get_cur_time();
      // receive_time += t3-temp_double[0];
      ...
      *(int *)(new_buffer+sizeof(int))=block_id;
      copy_msg_int((int*)(new_buffer+(1+1)*sizeof(int)),(int*)gv->org_recv_buffer,gv->block_size/sizeof(int));
      ...
      *(int *)(new_buffer+sizeof(int))=block_id;
      ...
      MPI_Get_count(&status, MPI_INT, &recv_int);
      // recv_int=recv_int/sizeof(int);
      printf("Disk_msg_id:- recv_int=%d,gv->mpi_recv_progress_counter=%d\n",
  ...
  printf("Analysis Process %d Receive thread %d total time=%f, mpi_recv_progress_counter=%d, \
