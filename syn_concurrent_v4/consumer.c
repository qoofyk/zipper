...
void simple_verify(GV gv, LV lv, char* buffer,  int nbytes, int source, int block_id){
  ...
  for(i = 8; i < (nbytes/4-2); i=i+4) {
    for(j=0;j<gv->lp;j++){
      x = *(int *)(buffer+i);
      if(x!=block_id+i){
        printf("Synthetic concurrent %d Wrong! x = %d, get block_id = %d\n",
          gv->calc_counter,x, block_id);
        fflush(stdout);
        return;
      }
      tmp = sqrt((double)x);
...
void consumer_thread(GV gv,LV lv){
  ...
  while(1) {
    ...
    if (pointer == NULL) {
      ...
      // if(gv->rank[0]==gv->num_compute_nodes && gv->calc_counter%100==0)
      // if(gv->calc_counter%100==0)
  ...
  printf("Analysis Process %d #### consumer total ####total consumer time= %f,calc_time= %f, wait_time= %f, calc_counter=%d\n",
