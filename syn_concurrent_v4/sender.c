...
void sender_thread(GV gv,LV lv){
	  ...
	  //   	printf("Compute %d send a message send_counter=%d\n", gv->rank[0], send_counter);
	    ...
	    if(my_count >= gv->sender_blk_num){
			...
			if(writer_done_flag==1){
				...
				if(gv->send_tail>0){
		  			disk_msg_flag=1;
		  			disk_msg = (int*) malloc (sizeof(int)*gv->send_tail);
				...
				if(disk_msg_flag==1){
					...
					fflush(stdout);
					errorcode = MPI_Ssend(disk_msg,send_counter,MPI_INT,dest,DISK_TAG,MPI_COMM_WORLD);
		    		check_MPI_success(gv, errorcode);
	...
	printf("Comp%d Sender%d total_time=%f, mpi_send_prog=%d, \
