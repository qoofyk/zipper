...
void write_blk(GV gv, LV lv,int blk_id, char* buffer, int nbytes){
	...
	sprintf(file_name,ADDRESS,gv->num_compute_nodes, gv->num_analysis_nodes, gv->rank[0], gv->rank[0], blk_id);
	...
	while((fp==NULL) && (i<TRYNUM)){
	    ...
	    if(fp==NULL){
	    	if(i==TRYNUM-1){
	    		printf("Warning: Compute Process %d Writer %d write an empty file last_gen_rank=%d, blk_id=%d\n",
...
void writer_thread(GV gv,LV lv) {
	...
	printf("Compute Node %d Writer %d write_time= %f , only_fwrite_time=%f, total time= %f, with %d blocks\n",
