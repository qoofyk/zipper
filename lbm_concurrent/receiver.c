...
void copy_msg_double(double* temp1,double* temp2,int num_int){
...
void receiver_thread(GV gv,LV lv){
  ...
  while(1){
    ...
    if(status.MPI_TAG==MPI_MSG_TAG){
      ...
      *((int*)new_buffer) = status.MPI_SOURCE;
      *(int *)(new_buffer+sizeof(int))=block_id;
      *(int *)(new_buffer+sizeof(int)*2) = *((int *)(gv->org_recv_buffer+sizeof(int))); //step
      *(int *)(new_buffer+sizeof(int)*3) = *((int *)(gv->org_recv_buffer+sizeof(int)*2)); //CI
      *(int *)(new_buffer+sizeof(int)*4) = *((int *)(gv->org_recv_buffer+sizeof(int)*3)); //CJ
      *(int *)(new_buffer+sizeof(int)*5) = *((int *)(gv->org_recv_buffer+sizeof(int)*4)); //CK

      // int count=518448;
      // printf("Before copy, Analysis %d Receive %d get a block %d gv->org_recv_buffer[518448]=%f\n",
      //   gv->rank[0], lv->tid, block_id, *((double *)(gv->org_recv_buffer+20+count+8)) );
      // fflush(stdout);

      copy_msg_double((double*)(new_buffer+6*sizeof(int)),(double*)(gv->org_recv_buffer+sizeof(int)*5),gv->cubex*gv->cubey*gv->cubez*2);

      // printf("After copy, Analysis %d Receive %d get a block %d new_buffer[518448]=%f\n",
      //   gv->rank[0], lv->tid, block_id, *((double *)(new_buffer+24+count+8)) );
      // fflush(stdout);
      ...
      pthread_mutex_lock(&gv->lock_recv);
      // gv->prefetch_counter++;
      make_prefetch_id(gv, status.MPI_SOURCE, recv_int,temp_int_pointer);
      pthread_mutex_unlock(&gv->lock_recv);
      t5 = get_cur_time();
      wait_lock += t5-t4;
      ...
      *(int *)(new_buffer+sizeof(int)*5) = *((int *)(gv->org_recv_buffer+sizeof(int)*4)); //CK
      ...
      printf("mix_msg_id:- recv_int=%d,gv->mpi_recv_progress_counter=%d\n",
      ...
      printf("Ana Node %d receive error!\n", gv->rank[0]);
      fflush(stdout);
      exit(1);
    }
  }
...
receive_time = %f, wait_lock = %f, mix_msg_id= %d, disk_id=%d, long_msg_id=%d\n",
    gv->rank[0], lv->tid, t1-t0, gv->mpi_recv_progress_counter,
    receive_time, wait_lock, mix_msg_id, disk_id, long_msg_id);
