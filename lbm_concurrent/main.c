...
computer_group_size, num_analysis_nodes, cubex, cubez, step_stop, lp\n", argv[0]);
	...
	//init lbm
	gv->cubex=atoi(argv[7]);
	gv->cubey=atoi(argv[7]);
	gv->cubez=atoi(argv[8]);

	gv->X=nx/gv->cubex;
	gv->Y=ny/gv->cubey;
	gv->Z=nz/gv->cubez;

	gv->step_stop = atoi(argv[9]);
	gv->lp = atoi(argv[10]);
	...
	gv->block_size = sizeof(int)*4+sizeof(double)*(gv->cubex*gv->cubey*gv->cubez*2); //64K B+4B*4,step,ci,cj,ck,data

  	gv->cpt_total_blks = gv->X*gv->Y*gv->Z*gv->step_stop;
  	gv->writer_thousandth  = atoi(argv[4]);
  	...
  	gv->analysis_data_len = sizeof(int)*(1+1)+sizeof(char)*gv->block_size; // src,blkid,step,ci,cj,ck,data
  	// printf("writer_blk_num=%d, sender_blk_num=%d\n", gv->writer_blk_num, gv->sender_blk_num);
  	// fflush(stdout);

  	gv->computer_group_size=atoi(argv[5]);
  	gv->num_analysis_nodes=atoi(argv[6]);
	...
	if (gv->color == 0){
		        ...
		        for(j=0;j<gv->cubey;j++)
		          for(k=0;k<gv->cubez;k++){
		            ...
		            *((double *)(buffer+20+count+8))=u_r*v[gv->gi][gv->gj][gv->gk];
		            ...
		            // mean_u += u_r*u[gv->gi][gv->gj][gv->gk];
		            // mean_v += u_r*v[gv->gi][gv->gj][gv->gk];

		            // *((double *)(buffer+count+2))=gi;
		            // *((double *)(buffer+count+3))=gj;
		            // *((double *)(buffer+count+4))=gk;

		            // if( u_r*u[gv->gi][gv->gj][gv->gk]>0 || u_r*v[gv->gi][gv->gj][gv->gk]>0)
		    //         if( u_r*v[gv->gi][gv->gj][gv->gk]<0){
		    //         	printf("Compute Process %d u=%f v=%f, buffer1=%f, buffer2=%f, count=%d\n",
						// 	gv->rank[0], u_r*u[gv->gi][gv->gj][gv->gk], u_r*v[gv->gi][gv->gj][gv->gk],
						// 	*((double *)(buffer+20+count)), *((double *)(buffer+20+count+8)),count);
						// fflush(stdout);
		    //         }
		...
		// printf("Compute Process %d mean_u=%f mean_v=%f\n",
		// 	gv->rank[0], mean_u/(gv->cubex*gv->cubey*gv->cubez), mean_v/(gv->cubex*gv->cubey*gv->cubez));
		// fflush(stdout);
		...
		// gv->compute_all_done = 1;
		printf("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n");
		printf("Compute Process %d on %s LBM Compute Done! Only_LBM_Time=%f,Total_Compute_Time/Block=%fms, Total_Compute_Time=%f\n",
		gv->rank[0], gv->processor_name, only_lbm_time, (t3-t2)*1000/gv->cpt_total_blks,t3-t2);
		fflush(stdout);
		//------------------------------------------------END OF LBM--------------------------------------------------------
	    ...
	    if(gv->rank[0]==gv->num_compute_nodes || gv->rank[0]==(gv->num_compute_nodes+gv->num_analysis_nodes-1)){
	    	printf("Analysis Process %d ana_total_blks = %d\n", gv->rank[0], gv->ana_total_blks);
	    	printf("Analysis Process %d: CONSUMER_Ringbuffer %.3fGB, size is %d member\n",
