...
#ifdef ADD_NETWORK
			//print network info
			if(my_count%(gv->cpt_total_blks/10)==0 && (gv->rank[0]%gv->nproc_per_mac==0)){
				char command[256];
				printf("Comp_Proc%d: print network info at my_count=%d\n", gv->rank[0], my_count);
				fflush(stdout);
			   	sprintf(command, "/usr/sbin/opapmaquery -o getportstatus >> %s/Comp%d_Diskpercent%d", gv->filepath, gv->rank[0], gv->writer_thousandth);
				system(command);
				fflush(stdout);
				// sprintf(command, "/usr/sbin/opasmaquery -o conginfo >> %s/Comp%d_Diskpercent%d", gv->filepath, gv->rank[0], gv->writer_thousandth);
				// system(command);
				// fflush(stdout);
			}
#endif
