...
char get_disk_blk(GV gv, LV lv, int *last_gen_rank, int *block_id){
  ...
  while(1){
    ...
    if(gv->recv_avail>0){
      //printf("Reader %d read recv_tail = %d\n", lv->tid, gv->recv_tail);
      ...
      *block_id = gv->prefetch_id_array[gv->recv_tail+1];
      ...
      //   printf("!!!!!!!!!Node %d Reader %d get lock, last_gen_rank = %d, \n", gv->rank[0], lv->tid, last_gen_rank,);
      gv->recv_tail+=2;
      gv->recv_avail-=2;
      //printf("Now, Node %d Reader %d minus tail=%d\n", gv->rank[0],lv->tid,gv->recv_tail);
      pthread_mutex_unlock(&gv->lock_recv_disk_id_arr);
      return 1;
    }
    else{//gv->recv_avail==0
      if(gv->recv_exit == 1){
        gv->reader_exit=1;
        return 0;
      }
      pthread_cond_wait(gv->reader_on, &gv->lock_recv_disk_id_arr);
    }
...
void analysis_reader_thread(GV gv,LV lv) {
  ...
  if(gv->reader_blk_num==0){
    ...
    while(1){
      ...
      t0 = MPI_Wtime();
      flag = get_disk_blk(gv, lv, &last_gen_rank, &block_id);
      t1 = MPI_Wtime();
...
T_put=%.3f, T_Darr_wt=%.3f, cnt=%d, wait=%d\n",
    gv->rank[0], lv->tid, t3 - t2, lv->read_time, lv->only_fread_time,
    lv->ring_buffer_put_time, disk_arr_wait_time, read_file_cnt, lv->wait);
  fflush(stdout);
}
