...
void analysis_reader_thread(GV gv, LV lv) {
  ...
  char flag=0;
  int dump_lines_in_this_blk=0;
  int read_bytes=0;

  int recv_avail=0;
  double disk_arr_wait_time=0;

  // printf("Ana_Proc%d: Reader%d is running!\n",gv->rank[0], lv->tid);
  // fflush(stdout);
  ...
  if(gv->reader_blk_num==0){
    if(gv->rank[0]==gv->compute_process_num || gv->rank[0]==(gv->compute_process_num+gv->analysis_process_num-1)){
      printf("Ana_Proc%04d: Reader%d is turned off\n", gv->rank[0], lv->tid);
      fflush(stdout);
    }
    ...
    while (1){
      ...
      if (gv->recv_tail>0){
        ...
        //LAMMPS_CHANGE: start from gv->recv_tail+3
        ...
        dump_lines_in_this_blk = gv->prefetch_id_array[gv->recv_tail+2];
...
#ifdef DEBUG_PRINT
        printf("Ana_Proc%d: Reader%d starts read src=%d blk_id=%d dump_lines_in_this_blk=%d\n",
          gv->rank[0], lv->tid, last_gen_rank, block_id);
        fflush(stdout);
#endif //DEBUG_PRINT

        t0 = MPI_Wtime();
        read_bytes = sizeof(double)*dump_lines_in_this_blk*5+sizeof(int)*2;
#ifdef WRITE_ONE_FILE
        ana_read_one_file(gv, lv, last_gen_rank, block_id, new_buffer+4*sizeof(int), gv->ana_read_fp[last_gen_rank%gv->computer_group_size], read_bytes);
#else
        read_blk_per_file(gv, lv, last_gen_rank, block_id, new_buffer+4*sizeof(int), read_bytes);    //read file block to buffer memory
#endif //WRITE_ONE_FILE
        t1 = MPI_Wtime();
        lv->read_time += t1 - t0;
        read_file_cnt++;
