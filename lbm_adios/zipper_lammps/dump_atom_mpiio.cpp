...
#include <VT.h>
static int class_id;
static int advance_step_id;// get_buffer_id;
#endif
...
/* ---------------------------------------------------------------------- */
void DumpAtomMPIIO::insert_into_DataBroker(int nme, double * buf)
{
  int i,j;
  double xs, ys, zs;
  double atom_id, type;
  ...
  total_num_double = nme * size_one;
  num_blk_cur_step = ceil(((double)nme)/((double)gv->dump_lines_per_blk));
  ...
  if(gv->rank[0]==0){
    ...
    printf("Comp_Proc%04d: timestep=%d dump %d lines, generate %d Zipper_Blks with size_one=%d\n",
      gv->rank[0], update->ntimestep, nme, num_blk_cur_step, size_one);
    fflush(stdout);
#endif //DEBUG_PRINT
  }
  ...
  int cur_line=0;
  for(i=0; i<total_num_double; i+=5){
...
// #endif //DEBUG_PRINT
    ...
    ((double *)(buffer+sizeof(int)*3))[j]   = buf[i];
    ((double *)(buffer+sizeof(int)*3))[j+1] = buf[i+1];
    ((double *)(buffer+sizeof(int)*3))[j+2] = buf[i+2]*domain->xprd+boxxlo;
    ((double *)(buffer+sizeof(int)*3))[j+3] = buf[i+3]*domain->yprd+boxylo;
    ((double *)(buffer+sizeof(int)*3))[j+4] = buf[i+4]*domain->zprd+boxzlo;
    ...
    if( (cur_line)%gv->dump_lines_per_blk==0 || cur_line>=nme ){
...
void DumpAtomMPIIO::write()
{
...
#ifdef TURN_ON_LAMMPS_MPIIO //TURN OFF lammps MPIIO
  ...
  performEstimate = 0;
  // write_data(nme,buf);
#endif //TURN_ON_LAMMPS_MPIIO

  insert_into_DataBroker(nme, buf);
