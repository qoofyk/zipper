...
CMD_PRODUCER="$LAUNCHER -n $PROCS_PRODUCER -o ${PROCS_SERVER} ${BIN_PRODUCER} ${NSTOP} ${FILESIZE2PRODUCE}"
$CMD_PRODUCER  &> ${PBS_RESULTDIR}/producer.log &
echo "producer applciation lauched: $CMD_PRODUCER"

CMD_CONSUMER="$LAUNCHER -n $PROCS_CONSUMER -o $((${PROCS_SERVER}+${PROCS_PRODUCER})) ${BIN_CONSUMER} ${NSTOP} ${FILESIZE2PRODUCE} ${PROCS_PRODUCER}"
