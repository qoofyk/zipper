...
void calc_n_moments(GV gv, LV lv, char* buffer, char* consumer_state_p){
  ...
  gv->step = ((int *)buffer)[3];
  gv->CI   = ((int *)buffer)[4];
  gv->CJ   = ((int *)buffer)[5];
  gv->CK   = ((int *)buffer)[6];
  #ifdef DEBUG_PRINT
  printf("Ana_Proc%d: consumer%d calc_n_moments computeid = %d, check_blkid=%d, source=%d, block_id=%d, \
step=%d, i=%d, j=%d, k=%d, gv->calc_counter=%d, *consumer_state_p=%d\n",
          gv->rank[0], lv->tid, gv->computeid, check_blkid, source, block_id,
          gv->step, gv->CI, gv->CJ, gv->CK, gv->calc_counter,*consumer_state_p);
  #endif //DEBUG_PRINT
  ...
  for(base=0;base<gv->cubex*gv->cubey*gv->cubez;base=base+4){
    u=((double *)(buffer+28))[base];
    v=((double *)(buffer+28))[base+1];
    u1=((double *)(buffer+28))[base+2];
    v1=((double *)(buffer+28))[base+3];
