...
void compute_sender_thread(GV gv,LV lv){
	...
	while(1){
	  ...
	  //   	printf("Compute %d send a message send_counter=%d\n", gv->rank[0], send_counter);
	    ...
	    if(my_count >= gv->sender_blk_num){
			...
			if(disk_msg_flag==1){
				printf("---###---Special case: Sender wait for Writer and send the last msg with %d blocks!!!!---###---\n",
					gv->send_tail);
				fflush(stdout);
	...
	printf("Compute Process %d Send thread %d total time is %f, mpi_send_progress_counter=%d,\
