...
for((i=0;i<${#cubex[@]};i++));do
	val=$((${cubex[i]} * ${cubex[i]} * ${cubez[i]} * 16 / 1024))
	...
	for ((k=0; k<${#writer_thousandth[@]}; k++)); do
		...
		for ((p=0; p<$maxp; p++)); do
			...
			real_run="$my_run_exp1 $compute_generator_num $compute_writer_num $analysis_reader_num $analysis_writer_num ${writer_thousandth[k]} $compute_group_size $num_ana_proc ${cubex[i]} ${cubez[i]} $NSTOP $n_moments $FILESIZE2PRODUCE ${writer_prb_thousandth[k]}"
