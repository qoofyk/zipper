...
void run_lbm(GV gv, int dims_cube[3], MPI_Comm *pcomm){
	...
	FILE *fp_out;
	double t0=0, t1=0,t2=0, t3=0,t4=0,t5=0,t6=0,only_lbm_time=0,init_lbm_time=0,mpi_sendrecv_time=0;
	int errorcode;
...
#ifdef DEBUG_PRINT
	printf("Comp_Proc%d: Start LBM data MPI_Sendrecv! step=%d\n", gv->rank[0], gv->step);
	fflush(stdout);
#endif //DEBUG_PRINT
...
#ifdef DEBUG_PRINT
	printf("Comp_Proc%d: LBM finish MPI_Sendrecv! step=%d\n", gv->rank[0], gv->step);
	fflush(stdout);
#endif //DEBUG_PRINT
...
#ifdef debug_1
                printf("(%d %d %d), u_r=_%lf u=%lf v= %lf\n", gv->gi, gv->gj, gv->gk, u_r, u[gi][gj][gk],v[gi][gj][gk]);
#endif
	            count+=2;
	          }
...
computer_group_size, num_analysis_nodes, cubex, cubez, step_stop, lp, filesize2produce\n", argv[0]);
		exit(1);
	}
